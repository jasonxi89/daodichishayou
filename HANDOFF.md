...
- **🚩 送审中（2026-09-01）：代码 v1.11.0 已上传微信后台并提交审核**。
  ...
  - **Owner 裁决（2026-09-01）：本版不撤回，后台就保留 1.9.3**，靠上面的映射关系兜住（已提交的版本号无法修改，撤回审核每月有次数限制，不值得为改号消耗）。
  - **下次上传务必手动改版本号**：弹窗会在 1.9.3 基础上续填 **1.9.4**，需手动改成与 `package.json` 一致的值（届时应为 1.12.0 或更高），以恢复两边对齐。这是本次错位的直接成因，别再踩。
  - 待回填：审核通过日期、是否已发布。
